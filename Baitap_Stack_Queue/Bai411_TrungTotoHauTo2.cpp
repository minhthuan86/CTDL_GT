...
typedef Node * Stack;
typedef Nut * GT;
typedef NodeS * StackS;
NodeS *TaoNodeS(int a){
    NodeS *p = new NodeS;
    p->data = a;
    p->next =NULL;
    return p;
}
void InitS(StackS &pHead){
    pHead=NULL;
}
void PushS(StackS &pHead,int a){
    StackS p=TaoNodeS(a);
    p->next=pHead;
    pHead=p;
}
int PopS(StackS &pHead){
    int a=pHead->data;
    StackS p=pHead;
    pHead=pHead->next;
    delete p;
...
void OutPutStackS(StackS pHead){
    StackS P=NULL;
    while(pHead!=NULL){
        PushS(P,pHead->data);
        pHead=pHead->next;
...
void DinhGia(Stack &Q,StackS &SS,GT &MyGT){
    int a,b;
    NhapGT(MyGT);
    //InGT(MyGT);
    while(MyGT!=NULL){
    ...
    do{
        int ch=(int) Pop(Q)-48;
        //cout<<"\n"<<ch<<"\n";
        if(ch>=0 && ch<=9) PushS(SS,ch);
        else {
            int val1= PopS(SS);
            int val2= PopS(SS);
            switch(ch) {
            case -5: PushS(SS, val2 + val1); break;
            case -3: PushS(SS, val2 - val1); break;
            case -6: PushS(SS, val2 * val1); break;
            case -1: PushS(SS, val2/val1); break;
            case 46: PushS(SS, pow(val2, val1)); break;
            }
        }
        OutPutDS(Q);
        cout << "\t\t";
